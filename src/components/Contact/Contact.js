...
export default function Contact() {
    ...
    const service = process.env.REACT_APP_SERVICE_KEY
    const template = process.env.REACT_APP_TEMPLATE_ID
    const publicKey = process.env.REACT_APP_PUBLIC_KEY

    const sendEmail = (e) => {
        e.preventDefault()
        emailjs.sendForm(service, template, refForm.current, publicKey)
        .then(()=>{
        ...
        (error) => {
            ...
            alert('Erreur dans l\'envoi du message, veuillez réessayer ultérieurement')
        });
    }
  return (
    <>
